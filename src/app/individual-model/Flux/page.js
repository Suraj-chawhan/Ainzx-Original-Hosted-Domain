...
import { FiUser, FiLogIn, FiArrowLeft, FiMenu, FiX } from "react-icons/fi";
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col md:relative w-full md:flex md:flex-row h-[100vh] font-sans bg-gray-50 dark:bg-gray-900 dark:text-gray-200">
      <div
        className="sm:hidden md:block absolute  top-4 right-4 md:flex flex-row  z-50 gap-4 md:gap-6 "
        ref={navRef}
      >
        <DarkModeToggle />
        {!session ? (
      ...
      <div
        ref={ref}
        className=" hidden md:block md:w-1/4 bg-blue-600 text-white dark:bg-gray-800 dark:text-gray-200 flex flex-col items-center p-8 shadow-xl"
      >
        <header className="mb-6 w-full text-center">
          <h1 className="text-4xl font-extrabold tracking-wide">Flux Chat</h1>
        </header>
        {/* Additional sidebar content */}
      </div>

      <div className="mt-0 flex-1 md:mt-14 p-0">
        <FluxChatTab />
      </div>
    </div>
  );
